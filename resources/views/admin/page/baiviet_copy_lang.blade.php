...
@section('content')
	...
	<form action="baiviet/add" method="post">
		<input type="hidden" class="form-control" name="lang"  value="{{$lang}}">
			<input type="hidden" class="form-control" name="origin_id"  value="{{$Post->id}}">
			<div class="boxwhite">
				<div class="row">
					<div class="col-md-8">
                    	...
                    	<div class="form-group">
							...
							<input type="text" required="required" 	class="form-control" placeholder="Vui lòng nhập tên sản phẩm" value="{{$Post->Post_Title}}" name="Post_Title"  />
						</div>
                    	<div class="form-group">
							<label><i class="fa fa-check-circle-o" aria-hidden="true"></i> link (đường dẫn bài viết)</label>
							<input type="text" class="form-control" value="" placeholder="ví dụ: ban-thich-choi-game-gi (có thể bỏ trống)" name="Post_Name"  />
						</div> 
						...
						<div class="row">	
							...
							<div class="col-md-3 trangthai">
								<div class="form-group">
									...
									<div class="box-radio">
										<label class="check"><input {{($Post->Post_Status==0) ? "checked" : ""}} type="radio" name="Post_Status"  value="0" class="form-radio"/> Ẩn</label>
										<label class="check"><input type="radio" {{($Post->Post_Status==1) ? "checked" : ""}} name="Post_Status" value="1" class="form-radio"/> Hiện</label>
									</div>
								</div>
							</div>
                          ...
                          <!------------------------------>						
							<div class="col-md-3 trangthai"  style="display:none;">
								<div class="form-group">
									...
									<input type="text" class="form-control" value="{{$Post->SoLuong}}" name="SoLuong" />
								</div>
							</div>
                         </div>       
                            <!------------------------------>	  
                             <div class="form-group mota">
                                ...
                                <textarea  class="form-control"  name="Short_Post_Content" rows="5" >{{$Post->Short_Post_Content}}</textarea>
                            </div>   
                            ...
                            <div class="form-group mota">
                                ...
                                <textarea  name="Post_Content" rows="5" >{{$Post->Post_Content}}</textarea>
                            </div>
					...
					<div class="col-md-4">
						...
						<label><i class="fa fa-check-circle-o" aria-hidden="true"></i> Hình đại diện </label>
							<div class="box-form">
								<input type="text" name="Post_Thumb"  id="avatar" readonly="" value="{{$Post->Post_Thumb}}" class="form-control">
            ...
            <div class="form-group">
					...
					<input type="text" class="form-control" value="{{$Post->Title}}" name="Title" placeholder="title để Seo"/>
				</div>
				<div class="form-group">
					<label><i class="fa fa-check-circle-o" aria-hidden="true"></i> Keywords</label>
					<textarea rows="2" type="text" class="form-control" placeholder="keywords meta để Seo" name="Keyword" >{{$Post->Keyword}}</textarea>
				</div>
				<div class="form-group">
					<label><i class="fa fa-check-circle-o" aria-hidden="true"></i> Description</label>
					<textarea rows="3" type="text" class="form-control" placeholder="Description meta để Seo" name="Desription" >{{$Post->Desription}}</textarea>
				</div>
			</div>
            </div>
